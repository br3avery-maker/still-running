...
Do not change established facts, invent lore, or edit story prose. Do not compress a range into an exact date, omit the `T0_BLACKOUT` / `T1_NIX_WAKE` fixed anchors, or erase open temporal debts from the handoff. Update related continuity files only when needed to keep classifications correct.
